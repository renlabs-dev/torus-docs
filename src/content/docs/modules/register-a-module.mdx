...
make sure that you [served](/mining/run-miner) it beforehand or no validator will be able to connect to it.

### Registering Through CLI

```bash
tor module register <module_name> <module_key> [--ip <integer>] [--port <integer>] [--netuid <integer>] [--stake <float>] [--metadata <text>] [--new-subnet-name]
```
...
```bash
tor module update <key-name> <netuid> [--name] [--ip] [--port] [--delegation-fee] [--metadata]
```
