...
- **`max_allowed_validators`**: The maximum number of validators allowed in the network (default: 128). Controls the maximum validators that can participate in the consensus.
...
- **`min_validator_stake`**: The minimum stake required to become a validator (default: 50,000 TORUS). Sets minimum stake threshold for validators.
...
- **`max_allowed_weights`**: The maximum number of weights allowed (default: 420). Sets upper limit on weight allocations per validator.

- **`min_allowed_weights`**: The minimum number of weights allowed (default: 1). Sets upper limit on weight allocations per validator.

- **`reward_interval`**: Blocks between reward distributions (default: 100). Determines how often rewards are distributed.

- **`immunity_period`**: Immunity period for new validators (default: 0 blocks). Grace period for newly registered validators.
