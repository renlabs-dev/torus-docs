...
```bash
tor balance transfer <key-name> <amount> <receiver-ss58address>
```

Or, if the receivers key is located on your disk:

```bash
tor balance transfer <key-name> <amount> <key-name>
```
...
```bash
tor balance stake <key-name> <amount> <validator-ss58address>
```

Similar to transferring if the validator key is already located on your disk you can use:

```bash
tor balance stake <key-name> <amount> <validator-name>
```
...
```bash
tor balance unstake <key-name> <amount> <validator-ss58address>
```
