...
The `AgentServer` class is a powerful framework for building authenticated APIs with automatic OpenAPI documentation. It provides blockchain-based authentication, namespace permissions, and seamless integration with the Torus Network.
...
The `agentKey` should be the SS58 address of your registered agent on the Torus Network. This is used for namespace permission checking.
</Aside>
...
The AgentServer integrates with Torus Network's namespace permission system for fine-grained access control.

### Basic Namespace Protection

```ts
agent.method("memory-write", {
  auth: { required: true },
  namespace: {
...
If no custom path is specified, the system generates paths using the format:
...
interface NamespaceOptions {
  /** Whether to enable namespace permission checking */
  enabled?: boolean; // Default: true
...
Users must have explicit namespace permissions granted by the agent owner to access protected endpoints. Permissions are managed through the Torus Network's permission system.
</Aside>
...
agent.method("analyze-text", {
  ...
  output: {
    ...
    err: {
      ...
      schema: z.object({
        error: z.string(),
        code: z.enum(["INVALID_INPUT", "PROCESSING_ERROR", "RATE_LIMIT"]),
...
The callback context provides helper functions for blockchain interactions:

```ts
agent.method("verify-transaction", {
  auth: { required: true },
  input: z.object({
    blockHash: z.string(),
  ...
  // Use the checkTransaction helper
  const result = await context.checkTransaction({
...
The AgentServer provides standardized error handling with appropriate HTTP status codes:

- **400**: Client errors (invalid input, business logic errors)
- **401**: Authentication errors (missing or invalid JWT)
- **403**: Authorization errors (insufficient namespace permissions)

```ts
agent.method("risky-operation", {
  auth: { required: true },
  input: z.object({
    action: z.string()
  }),
...
**Field Descriptions:**
- `sub`: User's SS58 address (42-character format)
...
After JWT verification, check namespace permissions:

1. **Generate Namespace Path**: `agent.<agent_name>.<endpoint_name>.<http_method>`
...
- **403 Forbidden**: Insufficient namespace permissions
...
- Use namespace permissions for sensitive operations
...
- Document namespace permission requirements
